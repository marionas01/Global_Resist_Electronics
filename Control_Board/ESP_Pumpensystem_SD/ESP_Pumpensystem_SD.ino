 ...
 *   SPI:  SCK -> GPIO4   MOSI -> GPIO6   MISO -> GPIO7   CS -> GPIO10 (TFT)
 *   TFT:  DC  -> GPIO2   RST  -> GPIO3   (pick free pins, adjust if wired differently)
 *   SD:   shares the same SPI bus as the TFT (SCK4/MOSI6/MISO7), own CS -> GPIO5
 *         (Micro-SD SPI breakout, e.g. the DAOKAI module - VCC/GND + SCK/MOSI/MISO/CS)
 *
 * SD logging:
 *   Every control cycle (every PID_SAMPLE_TIME_MS) the setpoint, filtered flow
 *   and drive voltage of every pump are appended as one CSV row to
 *   LOG_FILENAME on the SD card. If the card is missing or fails to mount,
 *   logging is silently skipped (sdReady=false) and the pump control/PID loop
 *   keeps running normally - a missing SD card must never stop the pumps.
 *   Required library: "SD" (bundled with the ESP32 Arduino core).
 *
...
#define SD_CS     5   // Micro-SD breakout, shares SPI bus with the TFT
...
// ------------------------------------------------------------
const double PID_KP = 0.010;
const double PID_KI = 0.010;
const double PID_KD = 0.000;

const double PID_OUT_MIN = 1.0;      // Volt
const double PID_OUT_MAX = 250.0;    // Volt (Highdriver-Maximum)

const unsigned long PID_SAMPLE_TIME_MS = 200;  // Regelungsfrequenz (wie bisher, 300 ms)
...
// ------------------------------------------------------------
// SD-Logging: Sensorwerte + Steuerspannung pro Pumpe, 1 Zeile pro Regelzyklus
// ------------------------------------------------------------
const char* LOG_FILENAME = "/pumplog.csv";
bool sdReady = false;              // false -> Logging wird stillschweigend übersprungen
uint32_t sdWriteErrorCount = 0;    // zählt fehlgeschlagene Schreibversuche (Diagnose)
...
// ============================================================
// SD-Karte: Init + CSV-Logging
// ============================================================
// Initialisiert die SD-Karte auf dem gemeinsamen SPI-Bus (gleiche
// SCK/MOSI/MISO-Leitungen wie das TFT, eigenes CS an SD_CS/GPIO5).
// tftSPI.begin() muss VOR diesem Aufruf bereits gelaufen sein.
// Schreibt bei Bedarf den CSV-Header. Gibt bei jedem Fehler false
// zurück (Karte fehlt/nicht formatiert/Schreibfehler) - die Pumpen-
// regelung läuft in diesem Fall trotzdem normal weiter, es wird nur
// nicht geloggt.
bool initSD() {
  ...
  if (!SD.exists(LOG_FILENAME)) {
    ...
    for (uint8_t p = 0; p < NUM_PUMPS; p++) {
      f.print(",P"); f.print(p + 1); f.print("_soll_ulmin");
      f.print(",P"); f.print(p + 1); f.print("_ist_ulmin");
      f.print(",P"); f.print(p + 1); f.print("_sensorOk");
      f.print(",P"); f.print(p + 1); f.print("_U_V");
    }
    f.println();
    f.close();
  }
...
// Hängt eine Zeile mit den aktuellen Werten aller Pumpen an die Logdatei an.
// Öffnet/schließt die Datei bei jedem Aufruf (statt sie dauerhaft offen zu
// halten), damit bei einem Stromausfall/Reset möglichst wenig Daten
// verloren gehen. Bei Schreibfehlern wird nur der Fehlerzähler erhöht;
// die Pumpenregelung ist davon nicht betroffen.
void logToSD() {
  if (!sdReady) return;
  ...
  f.print(millis());
  for (uint8_t p = 0; p < NUM_PUMPS; p++) {
    f.print(','); f.print(PUMP_SETPOINT_ULMIN[p], 1);
    f.print(','); f.print(lastGoodFlow[p], 2);
    f.print(','); f.print(pumpSensorOk[p] ? 1 : 0);
    f.print(','); f.print(pidOutput[p], 2);
  }
  f.println();
  f.close();
}
...
void drawStaticLayout() {
  ...
  // + SD-Schreibfehlerzähler (grün=0, orange>0, grau wenn keine Karte da)
  tft.setTextSize(1);
  tft.setTextColor(ST77XX_WHITE);
  tft.setCursor(originX, originY);
  tft.print("Fehler:");
  ...
  tft.setCursor(originX + 110, originY);
  tft.print("SD:");
  lastSdErrStr[0] = '\0';
...
// ---------------- Status bar: laufender I2C-Fehlerzähler + SD-Status ----------------
void updateStatusBar() {
  ...
  uint16_t sdColor;
  if (!sdReady) {
    snprintf(sdBuf, sizeof(sdBuf), "FEHLT");
    sdColor = ST77XX_RED;
  } else {
    snprintf(sdBuf, sizeof(sdBuf), "%lu", (unsigned long)sdWriteErrorCount);
    sdColor = (sdWriteErrorCount == 0) ? ST77XX_GREEN : ST77XX_ORANGE;
  }
  updateField(originX + 130, originY, 90, STATUS_BAR_H, 1, sdColor, sdBuf,
              lastSdErrStr, sizeof(lastSdErrStr));
}
...
void setup() {
  ...
  // SD-Karte auf demselben SPI-Bus wie das TFT initialisieren (eigenes CS
  // an GPIO5). Muss nach tftSPI.begin() erfolgen. Schlägt die Karte fehl,
  // läuft die Pumpenregelung trotzdem ganz normal weiter - es wird dann
  // nur nicht geloggt (siehe sdReady).
  sdReady = initSD();
  ...
  // I2C-Check direkt auf dem Display (Mux/Treiber/Sensoren/SD) - läuft
